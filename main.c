...
char initParameter(char **argv, int argc, char initValue);
void printHelp();
...
int main(int argc, char **argv) {
    ...
    for (i = 0; ((i < MAX_DEVICES) && (i < iNumDevs)); i++) {
        ...
        char rxBuffer[1];
        ftStatus = FT_Read(ftHandle[i], rxBuffer, 1, &bytesReceived);
        if (ftStatus != FT_OK || bytesReceived != 1) {
            printf("FT_Read failed (error %d).\n", (int) ftStatus);
            return ftStatus;
        }
         
        char inputParam = initParameter(argv, argc, rxBuffer[i]);
